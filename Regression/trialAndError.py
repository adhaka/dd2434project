# -*- coding: utf-8 -*-
"""
Created on Sat Jan 10 17:51:06 2015
...
def getDesMat(xinp, r):
    desMat = np.ones((len(xinp), len(xinp) + 1))
    for i in range(len(xinp)):
        desMat[i,0] = 1
        for j in range(len(xinp)):
            desMat[i,j+1] = kernel(xinp[i], xinp[j], r, BASIS)
    return desMat



def predictRVM(xtest, xinp, muMat, idx, r):
    ...
    for i in range(len(xtest)):
        ...
        for j in range(len(xinp)):
            xkernel[j+1] = kernel(xtest[i], xinp[j], r, BASIS)
